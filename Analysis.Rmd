...
- This is test the idea of mapping clusters
```{r warning=FALSE, message=FALSE}
...
#### Geocoding data with Northern Cape Tag
```{r warning=FALSE, message=FALSE}
...
```{r warning=FALSE, message=FALSE}
sapply(geo_df, function(x) sum(is.na(x)))
```


```{r warning=FALSE, message=FALSE}
geo_df[rowSums(is.na(geo_df)) > 0,]
```

#### removing NA
```{r warning=FALSE, message=FALSE}
locations_df2 <- subset(geo_df, !is.na(geo_df$longitude) & !is.na(geo_df$latitude))
```
...
```{r warning=FALSE, message=FALSE}
# write.csv(geo_df, "geo_NCdata2.csv")
```
